...
namespace XQ.Gateway.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Gateway") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Gateway", DisplayName = "Gateway API", Resources = { "Gateway" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["Gateway_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["Gateway_Web:RootUrl"]!.EnsureEndsWith('/');

            /* Gateway_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Gateway_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        var consoleAndAngularClientId = configurationSection["Gateway_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["Gateway_App:RootUrl"]?.TrimEnd('/');
            await CreateApplicationAsync(
        ...
        // Blazor Client
        var blazorClientId = configurationSection["Gateway_Blazor:ClientId"];
        if (!blazorClientId.IsNullOrWhiteSpace())
        {
            var blazorRootUrl = configurationSection["Gateway_Blazor:RootUrl"]?.TrimEnd('/');
        ...
        var blazorServerTieredClientId = configurationSection["Gateway_BlazorServerTiered:ClientId"];
        if (!blazorServerTieredClientId.IsNullOrWhiteSpace())
        {
            var blazorServerTieredRootUrl = configurationSection["Gateway_BlazorServerTiered:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Gateway_BlazorServerTiered:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Gateway_Swagger:ClientId"];
